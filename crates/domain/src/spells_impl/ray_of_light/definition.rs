...
impl RayOfLightSpell {
    ...
    pub fn targets_along_line(
        ...
        look_direction: Vec3,
        potential_targets: &[(EntityId, Vec3)],
        max_range: f32,
...
mod tests {
    ...
    fn targets_along_line_hits_every_entity_in_the_beam() {
        ...
        let near = EntityId::new(2);
        let far = EntityId::new(3);
        let targets = [
        ...
        let hit_entities: Vec<EntityId> = hits.iter().map(|(entity, _)| *entity).collect();
    ...
    fn targets_along_line_uses_horizontal_beam_hitbox() {
        let caster = EntityId::new(1);
        let elevated = EntityId::new(2);
        let offset_inside_hitbox = EntityId::new(3);
        let targets = [
    ...
    fn targets_along_line_ignores_targets_behind_or_off_axis() {
        let caster = EntityId::new(1);
        let behind = EntityId::new(2);
        let side = EntityId::new(3);
        let targets = [

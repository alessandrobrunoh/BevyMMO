//! Client visuals for weapon casts.
//!
...
/// Visual di un weapon ability, scelto dalla sua geometria.
...
pub fn animate(
    ...
    mut visuals: ParamSet<(
        Query<(Entity, &mut Transform, &mut AbilityBurst)>,
        Query<(Entity, &mut Transform, &mut AbilityGroundRing)>,
        Query<(Entity, &mut Transform, &mut AbilityFallingRock)>,
    )>,
) {
...
fn animate_bursts(
    delta: f32,
    commands: &mut Commands,
    bursts: &mut Query<(Entity, &mut Transform, &mut AbilityBurst)>,
) {
...
fn animate_ground_rings(
    delta: f32,
    commands: &mut Commands,
    rings: &mut Query<(Entity, &mut Transform, &mut AbilityGroundRing)>,
) {
...
fn animate_falling_rocks(
    delta: f32,
    commands: &mut Commands,
    rocks: &mut Query<(Entity, &mut Transform, &mut AbilityFallingRock)>,
) {

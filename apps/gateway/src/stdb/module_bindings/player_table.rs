...
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.player().normalized_name().find(...)`.
pub struct PlayerNormalizedNameUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<Player, String>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> PlayerTableHandle<'ctx> {
    /// Get a handle on the `normalized_name` unique index on the table `player`.
    pub fn normalized_name(&self) -> PlayerNormalizedNameUnique<'ctx> {
        PlayerNormalizedNameUnique {
            imp: self.imp.get_unique_constraint::<String>("normalized_name"),
            phantom: std::marker::PhantomData,
        }
    }
}

impl<'ctx> PlayerNormalizedNameUnique<'ctx> {
    /// Find the subscribed row whose `normalized_name` column value is equal to `col_val`,
    /// if such a row is present in the client cache.

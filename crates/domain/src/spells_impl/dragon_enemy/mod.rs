...
pub const ASH_RED: Rgba = Rgba::opaque(0.60, 0.05, 0.05);
/// Fire orange for impacts (sRGB `(1.00, 0.45, 0.05)`).
pub const FIRE_ORANGE: Rgba = Rgba::opaque(0.95, 0.40, 0.05);
/// Hot ember yellow for cores and sparks (sRGB `(1.00, 0.85, 0.25)`).
pub const EMBER_YELLOW: Rgba = Rgba::opaque(1.00, 0.70, 0.20);
/// Residual smoke after fire fades (sRGB `(0.25, 0.20, 0.18)`).
pub const SMOKE_GRAY: Rgba = Rgba::opaque(0.05, 0.04, 0.04);
/// Dusty tan for wing-buffet shockwaves (sRGB `(0.80, 0.70, 0.55)`).
pub const DUST_TAN: Rgba = Rgba::opaque(0.15, 0.13, 0.10);
